...
from toolbox.commands.utils import config_workspace, write_file
...
def export_module(module: str,
    ...
    pages = module_api.list_objects(workspace_name)
    if pages == [[]]:
    ...
    if module == "Dictionary":
            ...
            for source in page:
                ...
                if bulktree_mode:
                    ...
                    for batch_index, batch in enumerate(batches):
                        ...
                        bulktree = bulktree[0]
                        if len(batches) == 1:
                            filename = f"{workspace_name}_{module}_{source_name}_bulktree_{timestamp}.json"
                        else:
                            filename = f"{workspace_name}_{module}_{source_name}_bulktree_part{batch_index+1}_{timestamp}.json"
                        write_file(filename, export_dir, bulktree)

                else:
                    # Containers
                    for container_index, container_page in enumerate(containers):
                        if len(containers) == 1:
                            filename = f"{workspace_name}_{module}_{source_name}_1_containers_{timestamp}.json"
                        else:
                            filename = f"{workspace_name}_{module}_{source_name}_1_containers_part{container_index+1}_{timestamp}.json"
                        write_file(filename, export_dir, container_page)
                    # Structures
                    for structure_index, structure_page in enumerate(structures):
                        if len(structures) == 1:
                            filename = f"{workspace_name}_{module}_{source_name}_2_structures_{timestamp}.json"
                        else:
                            filename = f"{workspace_name}_{module}_{source_name}_2_structures_part{structure_index+1}_{timestamp}.json"
                        write_file(filename, export_dir, structure_page)
                    # Fields
                    for field_index, field_page in enumerate(fields):
                        if len(fields) == 1:
                            filename = f"{workspace_name}_{module}_{source_name}_3_fields_{timestamp}.json"
                        else:
                            filename = f"{workspace_name}_{module}_{source_name}_3_fields_part{field_index+1}_{timestamp}.json"
                        write_file(filename, export_dir, field_page)
        ...
        if module == "DataProcessing":
            handle_dpis(pages, module_api, workspace_name)
        ...
        for page_index, page in enumerate(pages):
            if bulktree_mode:
                ...
                if len(pages) == 1:
                    filename = f"{workspace_name}_{module}_bulktree_{timestamp}.json"
                else:
                    filename = f"{workspace_name}_{module}_bulktree_part{page_index+1}_{timestamp}.json"
                write_file(filename, export_dir, bulktree)
            else:
                if len(pages) == 1:
                    filename = f"{workspace_name}_{module}_{timestamp}.json"
                else:
                    filename = f"{workspace_name}_{module}_part{page_index+1}_{timestamp}.json"
                write_file(filename, export_dir, page)
